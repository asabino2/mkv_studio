...
const { detectSubtitleLanguage } = require('./languageDetector.js');
...
function scanDirectory(sourceDir, destDir) {
  if (!fs.existsSync(sourceDir)) {
    throw new Error(`A pasta de origem "${sourceDir}" não existe.`);
  }

  const stat = fs.statSync(sourceDir);
  if (!stat.isDirectory()) {
    throw new Error(`O caminho de origem "${sourceDir}" não é um diretório.`);
  }
  ...
  if (destDir && !fs.existsSync(destDir)) {
    try {
      ...
      throw new Error(`Não foi possível criar a pasta de destino "${destDir}": ${e.message}`);
    }
  }
