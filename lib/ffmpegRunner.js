...
class FFmpegWorker {
  ...
  convertItem(item, forcedEncoding, accelerator, onLog, onProgress) {
    ...
    return new Promise((resolve, reject) => {
      if (this.isCancelled) {
        return reject(new Error('Processing canceled by user.'));
      }
      ...
      const selectableAudios = (item.audioTracks || []).filter(audio => (audio.mode === 'selectable' || !audio.mode));

      onLog('info', `Starting video conversion: ${item.videoName}`);
      onLog('info', `Destination: ${item.destPath}`);
      onLog('info', `Selected Accelerator: ${accelerator || 'CPU'}`);
      onLog('info', `Soft Subtitles: ${selectableSubs.length} | Hard Subtitles (Burn): ${burnSub ? burnSub.srtName : 'None'} | Additional Audio Tracks: ${selectableAudios.length}`);

      selectableSubs.forEach((sub, idx) => {
        const effEnc = (forcedEncoding && forcedEncoding !== 'AUTO') ? forcedEncoding : (sub.encoding || 'utf-8');
        onLog('info', `  - [Selectable Subtitle ${idx + 1}] ${sub.srtName} -> Language: ${sub.langName} (${sub.langCode}) [Encoding: ${effEnc}]`);
      });

      if (burnSub) {
        const effEnc = (forcedEncoding && forcedEncoding !== 'AUTO') ? forcedEncoding : (burnSub.encoding || 'utf-8');
        onLog('info', `  - [Burn-in Subtitle] ${burnSub.srtName} -> Language: ${burnSub.langName} (${burnSub.langCode}) [Encoding: ${effEnc}]`);
      }

      selectableAudios.forEach((audio, idx) => {
        onLog('info', `  - [Selectable Audio ${idx + 1}] ${audio.audioName} -> Language: ${audio.langName} (${audio.langCode})`);
      });
      ...
      // Inputs para Legendas SRT selecionáveis
      selectableSubs.forEach(sub => {
        subInputIndices.push(nextInputIndex);
      ...
      // Inputs para Arquivos de Áudio selecionáveis
      selectableAudios.forEach(audio => {
      ...
      // Mapeamento de áudios adicionais
      audioInputIndices.forEach(inputIdx => {
      ...
      // Metadados das faixas de áudio adicionais
      if (selectableAudios.length > 0) {
        selectableAudios.forEach((audio, idx) => {
          args.push(`-metadata:s:a:${idx + 1}`, `language=${audio.langCode}`);
          args.push(`-metadata:s:a:${idx + 1}`, `title=${audio.langName}`);
        });
      }

      // Mapeamento das legendas selecionáveis (soft)
      if (selectableSubs.length > 0) {
        subInputIndices.forEach(inputIdx => {
          args.push('-map', `${inputIdx}:s`);
        });
      ...
      child.on('error', (err) => {
        ...
        onLog('error', `Error executing FFmpeg: ${err.message}`);
        reject(err);
      });
      ...
      child.on('close', (code) => {
        this.currentProcess = null;
        if (this.isCancelled) {
          onLog('warning', `Conversion canceled by user for: ${item.videoName}`);
          return reject(new Error('Conversion canceled by user'));
        }

        if (code === 0) {
          onLog('success', `Conversion completed successfully! MKV generated at: ${item.destPath}`);
          if (onProgress) onProgress(100);
          resolve(item.destPath);
        } else {
          onLog('error', `FFmpeg failed with exit code ${code} while processing ${item.videoName}`);
