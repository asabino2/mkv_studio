...
class FFmpegWorker {
  ...
  convertItem(item, forcedEncoding, accelerator, onLog, onProgress) {
    ...
    return new Promise((resolve, reject) => {
      ...
      onLog('info', `Legendas Soft: ${selectableSubs.length} | Legenda Hard (Burn): ${burnSub ? burnSub.srtName : 'Nenhuma'} | Áudios Adicionais: ${selectableAudios.length}`);

      selectableSubs.forEach((sub, idx) => {
        const effEnc = (forcedEncoding && forcedEncoding !== 'AUTO') ? forcedEncoding : (sub.encoding || 'utf-8');
        onLog('info', `  - [Legenda Selecionável ${idx + 1}] ${sub.srtName} -> Idioma: ${sub.langName} (${sub.langCode}) [Encoding: ${effEnc}]`);
      });

      if (burnSub) {
        const effEnc = (forcedEncoding && forcedEncoding !== 'AUTO') ? forcedEncoding : (burnSub.encoding || 'utf-8');
        onLog('info', `  - [Legenda Queimar no Vídeo] ${burnSub.srtName} -> Idioma: ${burnSub.langName} (${burnSub.langCode}) [Encoding: ${effEnc}]`);
      }

      selectableAudios.forEach((audio, idx) => {
        onLog('info', `  - [Áudio Selecionável ${idx + 1}] ${audio.audioName} -> Idioma: ${audio.langName} (${audio.langCode})`);
      });
